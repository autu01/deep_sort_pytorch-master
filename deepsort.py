...
# 核心补丁：给PyTorch原生Upsample类动态添加缺失的属性，适配PyTorch2.0
...
class VideoTracker(object):
    ...
    def run(self):
        ...
        while self.vdo.grab():
            ...
            # select vehicle classes (car, truck, bus, etc.)
            # COCO class IDs for vehicles: 2(car), 3(truck), 5(bus), 7(train)
            vehicle_classes = [2, 3, 5, 7]
            mask = np.isin(cls_ids, vehicle_classes)
            ...
            # do tracking
            if self.args.segment:
                outputs, mask_outputs, trajectories = self.deepsort.update(bbox_xywh, cls_conf, cls_ids, im, seg_masks)
            else:
                outputs, _, trajectories = self.deepsort.update(bbox_xywh, cls_conf, cls_ids, im)
            ...
            if len(outputs) > 0:
                ...
                ori_im = draw_boxes(ori_im, bbox_xyxy, names, identities, None if not self.args.segment else mask_outputs, trajectories)
